...
impl Participant {
    ...
    pub fn process_one<R: RngCore + CryptoRng>(
        ...
        match message.message_type {
            ...
            MessageType::RoundThree => {
                ...
                // FIXME: Make sure that we verify this message
                self.store(Storable {
                    storable_type: StorableType::RoundThreePublic,
                    associated_participant_id: message.from,
                    bytes: message.unverified_bytes,
                });
    ...
    fn do_round_one(&mut self) -> Result<Vec<Message>, Error> {
        ...
        for (other_id, r1_public) in r1_publics {
            ...
            self.store(Storable {
                ...
                associated_participant_id: other_id.clone(),
    ...
    fn validate_and_store_round_two_public(&mut self, message: &Message) -> Result<(), Error> {
        ...
        let receiver_r1_public_for_sender = crate::round_one::Public::from_slice(
            &self
...
mod tests {
    ...
    fn is_presigning_done(quorum: &[Participant]) -> Result<bool, Error> {
        for participant in quorum {
            if participant.is_ready_to_sign()? == false {
                return Ok(false);
            }
        }
        Ok(true)
